...
import sys
path = r'C:\Users\sigbj\Documents\Master\Code'
sys.path.append(path)
from heat.datasets.data_utils import RandomBlur
from heat.datasets.corners import CornersDataset
...
class OutdoorBuildingDataset(CornersDataset):
    def __init__(self, data_path, det_path, phase='train', image_size=256, rand_aug=True,
        ...
        if phase == 'train':
            datalistfile = data_path + r'\train_list.txt'
            self.training = True
        elif phase == 'valid':
            datalistfile = data_path + r'\val_list.txt'
            self.training = False
        elif phase == 'test':
            datalistfile = data_path + '/test_list.txt'
            self.training = False
    ...
    def __getitem__(self, idx):
        ...
        annot_path = os.path.join(self.data_path, 'annot', data_name + '.npy').replace('\\', '/')
        print(data_name)
        annot = np.load(annot_path, allow_pickle=True, encoding='latin1').tolist()
        det_path = os.path.join(self.det_path, data_name + '.npy').replace('\\', '/')
        det_corners = np.array(np.load(det_path, allow_pickle=True))  # [N, 2]
        det_corners = det_corners[:, ::-1]  # turn into x,y format

        img_path = os.path.join(self.data_path, 'rgb', data_name + '.jpg').replace('\\', '/')
